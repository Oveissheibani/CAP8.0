...
//      }
  ...
  {
  double density = 0.0;
  for (auto & nucleon __attribute__((unused)) : nucleus.allNucleons())
    {
    density += 1;
//    if (nucleon.isWounded() && nucleon.square2DDistanceTo(x,y)<=rMaxSq) density += dist.evaluate(sqrt(rSq));
  ...
  double GlauberHistos::calculateDensity(NuclearDistribution & dist __attribute__((unused)),
                                         ...
                                         double y __attribute__((unused)))
  {
  // Implementation is currently stubbed (the original body was commented out).
  // Returning 0.0 keeps the function deterministic until it is reimplemented.
  return 0.0;
  }
 ...
 // process nucleus A
  ...
  // typeA / typeB used to be needed for computing rMax but the maths was
  // moved elsewhere; keep the bindings only if a future change resurrects it.
  double impact = event.impactParameter();
  h_areaA->Reset();
  h_areaB->Reset();
